        ...
        if (!kDocOnly) {
            return listOf(
                TestParseNode(
                    ...
                    this,
                    collectNodes(kDocOnly = false)
                )
            )
        }

        // Ignore `MARKDOWN_LINK` for now because another subparser handles them
            ...
            elementType == KtTokens.DOC_COMMENT || insideKDoc || kDocStop -> {
                listOf(
                    TestParseNode(
                        ...
                        startOffset + textLength,
                        this,
                        collectNodes(kDocOnly = true, insideKDoc = !kDocStop)
                    )
                )
            }
            else -> {
                collectNodes(kDocOnly = true, insideKDoc = false)
            }
        }
    }

    fun PsiElement.collectNodes(kDocOnly: Boolean, insideKDoc: Boolean = false): List<TestParseNode<PsiElement>> {
        return buildList {
