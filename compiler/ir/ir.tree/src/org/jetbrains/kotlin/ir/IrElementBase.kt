...
abstract class IrElementBase : IrElement {
    ...
    private var indexAttributeRegistry: IrIndexBasedAttributeRegistry? = null

    internal fun <T : Any> getAttributeInternal(attribute: IrAttribute<T>): T? = when (attribute) {
        is IrIndexBasedAttribute<T> -> getAttributeInternal(attribute)
        is IrKeyBasedAttribute<*, T> -> getAttributeInternal(attribute)
    }

    internal fun <T : Any> setAttributeInternal(attribute: IrAttribute<T>, value: T?) = when (attribute) {
        is IrIndexBasedAttribute<T> -> setAttributeInternal(attribute, value)
        is IrKeyBasedAttribute<*, T> -> setAttributeInternal(attribute, value)
    }
    ...
    internal fun <T : Any> getAttributeInternal(attribute: IrIndexBasedAttribute<T>): T? {
    ...
    internal fun <T : Any> setAttributeInternal(attribute: IrIndexBasedAttribute<T>, value: T?) {
        if (indexAttributeRegistry == null) {
            indexAttributeRegistry = attribute.registry
        } else {
            assert(attribute.registry === indexAttributeRegistry) { "Attribute $attribute in registered in another registry" }
        }
        ...
        if (attributesSet and attributeMask == 0L) {
            ...
            val indexBasedAttributes = attributesSet.countOneBits()
    ...
    private fun <T : Any> initStorageWithOneIndexAttribute(attribute: IrIndexBasedAttribute<T>, value: T?) {
        if (value == null) {
            return
        }
    ...
    val indexBasedAttributes: Map<IrIndexBasedAttribute<*>, Any>
        get() {
            val attributeRegistry = indexAttributeRegistry ?: return emptyMap()
            var mask = indexAttributesSet
            ...
            return buildMap(mask.countOneBits()) {
                ...
                while (mask != 0L) {
                    ...
                    val attr = attributeRegistry.getById(id)
                    val value = storage[valueIndex++]!!
                    put(attr, value)
                }
            }
        }
    ...
    internal fun <T : Any> getAttributeInternal(attribute: IrKeyBasedAttribute<*, T>): T? {
        ...
        val low = indexAttributesSet.countOneBits() - 1
    ...
    private fun <T : Any> addAttributeAt(index: Int, attribute: IrKeyBasedAttribute<*, T>, value: T) {
        ...
        val indexBasedAttributes = indexAttributesSet.countOneBits()
        var newIndex = index
    ...
    private fun removeAttributeAt(keyIndex: Int) {
        ...
        val low = indexAttributesSet.countOneBits()
    ...
    internal fun copyKeyBasedAttributesFrom(other: IrElementBase, includeAll: Boolean) {
        ...
        val srcIndexBasedAttributes = indexAttributesSet.countOneBits()
        val dstIndexBasedAttributes = other.indexAttributesSet.countOneBits()
