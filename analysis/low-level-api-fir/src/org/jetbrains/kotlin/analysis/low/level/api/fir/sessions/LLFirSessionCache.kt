...
object LLFirSessionCacheInvalidator {
    ...
    class InvalidationSession(
        ...
        fun removeContextualDanglingFileSessions(contextModule: KaModule) {
            ...
            if (contextModule is KaDanglingFileModule) {
                removeAllMatchingSessionsFrom(storage.danglingFileSessionCache) { it is KaDanglingFileModule && hasContextModule(it, contextModule) }
            } else {
    ...
    inline fun performInvalidation(storage: LLFirSessionCacheStorage, invalidationAction: InvalidationSession.() -> Unit) {
        ...
        if (existingScope != null) {
            existingScope.invalidationAction()
        } else {
            val newScope = InvalidationSession(storage)
            currentInvalidationSession.set(newScope)
            try {
                newScope.invalidationAction()
            } finally {
                currentInvalidationSession.set(null)
            }
        }
    }

    val currentInvalidationSession = ThreadLocal<InvalidationSession?>()
}

class LLFirSessionCache(
    private val project: Project,
    internal val storage: LLFirSessionCacheStorage,
) : Disposable {
